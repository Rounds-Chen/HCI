...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if ((Input.GetMouseButtonDown(0) ||Input.GetKeyDown(KeyCode.LeftArrow))&& isJumping == false && nextPlatformLeft != Vector3.zero)
        {
            if (isMove == false)
            {
        ...
        else if((Input.GetMouseButtonDown(1) ||Input.GetKeyDown(KeyCode.RightArrow))&& isJumping == false && nextPlatformLeft != Vector3.zero)
        {
            if (isMove == false)
            {
                EventCenter.Broadcast(EventDefine.PlayerMove);
